...
ENV HOME=/go/src/github.com/harvester/node-manager

# ---- base ----
FROM builder AS base
WORKDIR /go/src/github.com/harvester/node-manager
COPY . .
...
RUN --mount=type=cache,target=/go/pkg/mod,id=harvester-node-manager-go-mod-${MK_REPO_ID} \
    --mount=type=cache,target=/go/src/github.com/harvester/node-manager/.cache/go-build,id=harvester-node-manager-go-build-${MK_REPO_ID} \
    ./scripts/build

FROM scratch AS build-output
COPY --from=build /go/src/github.com/harvester/node-manager/bin/ /bin/
...
COPY --from=generate /go/src/github.com/harvester/node-manager/pkg/ /pkg/
...
COPY --from=generate-manifest /go/src/github.com/harvester/node-manager/manifests/ /manifests/
